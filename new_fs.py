...
def points_to_desmos(X, Y, max_terms=30):
    ...
    ax, bx = fourier_series(f(x)[:,0])
    As_x = []
    Bs_x = []
    for i in range(max_terms):
        As_x.append(fr"{ax[i]:.6f}")
        Bs_x.append(fr"{bx[i]:.6f}")

    ay, by = fourier_series(f(x)[:,1])
    As_y = []
    Bs_y = []
    for i in range(max_terms):
        As_y.append(fr"{ay[i]:.6f}")
        Bs_y.append(fr"{by[i]:.6f}")

    return fr"\left(\operatorname{{total}}\left(\left[{', '.join(As_x)}\right]\cos\left(2\pi Nt\right)\right)+\operatorname{{total}}\left(\left[{', '.join(Bs_x)}\right]\sin\left(2\pi Nt\right)\right),\operatorname{{total}}\left(\left[{', '.join(As_y)}\right]\cos\left(2\pi Nt\right)\right)+\operatorname{{total}}\left(\left[{', '.join(Bs_y)}\right]\sin\left(2\pi Nt\right)\right)\right)"
